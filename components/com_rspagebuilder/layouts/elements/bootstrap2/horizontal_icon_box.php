...
$class				= 'rspbld-horizontal-icon-box';
...
if (!empty($element_options['icon'])) {
	...
	// Horizontal Icon Box icon style
	$icon_style = array();
	...
	// Horizontal Icon Box icon positioning
	if (!empty($element_options['icon_position'])) {
		$element_options['icon_position'] = ' pull-'.$element_options['icon_position'];
	}
}

// Horizontal Icon Box title
if (!empty($element_options['title']) && !empty($title_show)) {
	
	// Horizontal Icon Box title style
	$title_style = array();
...
// Horizontal Icon Box content
if (!empty($element_options['content'])) {
	
	// Horizontal Icon Box content style
	$content_style = array();
...
// Build Horizontal Icon Box HTML
...
<div class="<?php echo $class; ?>"<?php echo RSPageBuilderHelper::buildStyle($style); ?>>
	...
	<div class="rspbld-icon<?php echo $element_options['icon_position']; ?>"<?php echo RSPageBuilderHelper::buildStyle($icon_style);?>>
		<i class="fa fa-<?php echo $element_options['icon']; ?>"></i>
	</div>
	<?php } ?>
	
	<?php if ((!empty($element_options['title']) && !empty($title_show)) || !empty($element_options['content'])) { ?>
