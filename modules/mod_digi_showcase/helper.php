...
class Digi_Showcase_Helper {
	...
	public function __construct($dataSource=0, $dataFilter=0, $rows=1, $columns=1, 
								...
								$showExtraInfo, 
								$showFeaturedItems, $showExpiredItems, $itemsTimeCorrection) {
 ...
 */
	...
	public static function getImage($imageArray,$item) {
		...
		if ($show) {
			...
			if ($placeholderImage) {
				$placeholderPath = JUri::root().$placeholderImage;
			} else {
				$placeholderPath = JUri::root().'modules'.DS.'mod_digi_showcase'.DS.'assets'.DS.'images'.DS.'no-image.png';
			}
 ...
 */
	...
	public static function getFilterHTML($filterArray) {
		...
		if ($show) {
			...
			$html .= '<ul id="digi_showcase_filter" class="nav nav-pills">';
	
			$html .= '<li class="active"><a href="#" title="'.$keyword.'-all">'.jText::_('MOD_DIGI_SHOWCASE_FIELD_FILTER_ALL_LABEL').'</a></li>';
				...
				foreach($data As $filter) {
					...
					if ($filter || $filter == 0) {
						...
						if ($group && !in_array($filterTitle,$filtersArray) || !$group) {
							$html .= '<li><a href="#" title="'.$keyword.'-'.$filter.'">'.$filterTitle.'</a></li>';
						}
		...
		if ($show) {
			...
			$js .= '// give the correct position to filters
					...
					if (filterAlignment == 1 && ('.$jQuery.'(window).width() >= 769)) {
						...
						'.$jQuery.'("#digi_showcase_'.$id.' #digi_showcase_filter").addClass("pull-left");
					} else if (filterAlignment == 2) {
						'.$jQuery.'("#digi_showcase_'.$id.' #digi_showcase_filter").addClass("pull-right");
 ...
 */
	...
	public function getJoomlaArticles() {
        ...
        if ($this->order_by != "rand()") {
            ...
            $query .= sprintf(" ORDER BY %s %s LIMIT %d", $this->order_by, $this->order_type, $limit);
        }
 ...
 */
	...
	protected static function getResizedImage($itemId, $image_fullpath, $imageWidth='120', $imageHeight='80') {
        ...
        $vikresizer->croppedCenterImage($temp_imagepath, $dest_imagepath, $imageWidth, $imageHeight);
