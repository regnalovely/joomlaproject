...
if(!defined('DS')) define('DS', DIRECTORY_SEPARATOR);
    ...
    // return current joomla version from joomla manifest file
    function getJoomlaVersion() {
        $manifest_path = JPATH_ROOT.DS.'administrator'.DS.'manifests'.DS.'files'.DS.'joomla.xml';
        if (JFile::exists($manifest_path)) {
            $manifest = JFactory::getXML($manifest_path);
            $joomla_version = (string) $manifest->version;
            return $joomla_version;
        } else {
        	return 'missing Joomla manifest file';
        }
    }
    ...
    // return extension version type (free - lite - full)
    function getExtensionVersionType() {
    	...
    	if ($extension_family == 'components') {
    		...
    		$manifest_path = JPATH_ROOT.DS.$extension_family.DS.$plugin_type.DS.$extension_system_name.DS.$extension_system_name.'.xml';
    	} else {
    		$manifest_path = JPATH_ROOT.DS.$extension_family.DS.$extension_system_name.DS.$extension_system_name.'.xml';
    	}
		
    	if (JFile::exists($manifest_path)) {
            $manifest = JFactory::getXML($manifest_path);
            $extension_version_type = (string) $manifest->versiontype;
            return $extension_version_type;
        } else {
        	return 'Missing extension manifest file';
        }
    }
