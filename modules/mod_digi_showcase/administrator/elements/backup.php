...
class JFormFieldBackup extends JFormField {
    ...
    protected function getInput() {
    	...
    	$document->addScriptDeclaration('
			$j(document).ready(function(){
				...
				function backupOperation(current_operation) {
					...
					backup_message_container.html("<div class=\"alert alert-block alert-info\"><h4 style=\"text-transform: uppercase;\">'.JText::_(strToUpper($extension_system_name).'_FIELD_BACKUP_PLEASE_WAIT').'</h4> <span id=\"loader\"><img style=\"margin-left: 30px;\" width=\"14\" src=\"http://www.digigreg.com/images/icons/loader.gif\" alt=\"loading\"></span></div>");
					...
					$j.ajax({
						...
						success: function(){
							if (current_operation == "save") {
								...
								backup_message_container.html("<div class=\"alert alert-block alert-success\"><a href=\"#\" class=\"close\" data-dismiss=\"alert\">&times;</a><h4 style=\"text-transform: uppercase;\">'.JText::_(strToUpper($extension_system_name).'_FIELD_BACKUP_DONE').'</h4> '.JText::_(strToUpper($extension_system_name).'_FIELD_BACKUP_CURRENT_SETTINGS_SAVED').' \"" + current_file + ".json\".</div>");
        ...
        // load the results
        $load_file = JHtml::_('select.genericlist', $list, 'name', '', 'value', 'text', 'default', 'backup_load_filename');
        $delete_file = JHtml::_('select.genericlist', $list, 'name', '', 'value', 'text', 'default', 'backup_delete_filename');
        
        $html = '';
        $html .= '<div id="backup_form" class="well">';
        $html .= '<h2>'.JText::_(strToUpper($extension_system_name).'_FIELD_BACKUP_TITLE').'</h2>';
        $html .= '<p class="lead">'.JText::_(strToUpper($extension_system_name).'_FIELD_BACKUP_DESC').'</p>';
        $html .= '<div class="input-append"><p><span class="label label-success">'.JText::_(strToUpper($extension_system_name).'_FIELD_BACKUP_SAVE_SETTINGS').'</span></p><input type="text" id="backup_save_filename" class="input-medium" placeholder="'.JText::_(strToUpper($extension_system_name).'_FIELD_BACKUP_SAVE_PLACEHOLDER').'" /><span class="add-on">.json</span><button id="backup_save" class="btn btn-success"><em class="icon-file-add"></em> '.JText::_(strToUpper($extension_system_name).'_FIELD_BACKUP_SAVE').'</button></div>';
        $html .= '<hr /><div class="input-append"><p><span class="label label-info">'.JText::_(strToUpper($extension_system_name).'_FIELD_BACKUP_LOAD_SETTINGS').'</span></p>'.$load_file.'<button id="backup_load" class="btn btn-info"><em class="icon-file-check"></em> '.JText::_(strToUpper($extension_system_name).'_FIELD_BACKUP_LOAD').'</button></div>';
        $html .= '<hr /><div class="input-append"><p><span class="label label-danger">'.JText::_(strToUpper($extension_system_name).'_FIELD_BACKUP_DELETE_SETTINGS').'</span></p>'.$delete_file.'<button id="backup_delete" class="btn btn-danger"><em class="icon-file-remove"></em> '.JText::_(strToUpper($extension_system_name).'_FIELD_BACKUP_DELETE').'</button></div>';
        $html .= '<hr /><div id="backup_message" class="system-message" type="'.$extension_type.'" extension="'.$extension_name.'" version="'.$extension_version.'" joomlaversion="'.$joomla_version.'"></div>';
        $html .= '</div>';
