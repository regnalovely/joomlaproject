var RSPageBuilderHelper = {
	...
	duplicateItem: function(obj) {
		...
		clone.find('.rspbld-input').each(function() {
	...
	initEditors: function(obj) {
		obj.find('.editor').each(function() {
			var new_id = 'editor-' + RSPageBuilderHelper.randomNumber();
			
			jQuery(this).attr('id', new_id);
			tinyMCE.EditorManager.execCommand('mceAddEditor', false, new_id);
		});
	},
	...
	initTinyMCE: function() {
		var dir  = jQuery('html').attr('dir'),
		    lang = jQuery('html').attr('lang').split('-');
		...
		tinyMCE.init({
			...
			fontsize_formats: '8px 10px 12px 14px 16px 18px 20px 22px 24px 26px 28px 30px 32px 34px 36px 38px 40px 42px 44px 48px',
			...
			toolbar2: 'fontsizeselect | outdent indent | undo redo | link unlink anchor image code | hr table | subscript superscript | charmap',
			...
			setup: function(ed) {
				ed.on('change', function(e) {
					...
					jQuery('#' + ed.id).text(ed.getContent());
	...
	buildElementJson: function(element_container) {
		...
		element_container.find('.element-options .tab-content > div:not(#items) .rspbld-input').each(function() {
			if (jQuery(this).data('name') == 'type') {
				...
				element_options[jQuery(this).data('name')] = jQuery(this).val();
			}
		});
		...
		element_container.find('.element-options .iterative-items .accordion-inner').each(function(index) {
			...
			jQuery(this).find('.rspbld-input').each(function() {
				var field = jQuery(this).data('name').split('-');
				
				item_options[field[0]] = jQuery(this).val();
			});
